...
Short, in order. Pick → tilt → place → spin → **draw liquid first,
then attach the filter** → push.
...
8. Pick up the **bare syringe** (no filter attached yet).
9. Dip its Luer tip into the top of the post-spin tube and pull the plunger back to draw up the clear supernatant.
10. **Now** pick up the 0.45 µm syringe filter (it is sitting separately on the bench, NOT pre-attached) and screw it onto the syringe's Luer tip.
...
| 8 | **Syringe (10 mL)** — bare, no filter | BD Plastipak Luer-Lok 10 mL (P/N 309604) | Barrel Ø16 × 90 + plunger Ø11 × 70 + Ø22 thumb pad + Ø6 × 8 Luer tip; total ~140 | (0.10, +0.05) lying along Y | Drawn **without** the syringe filter attached. The arm uses the bare syringe to draw the supernatant first; the filter is screwed on only after the syringe is loaded. |
| 9 | **Syringe filter** (parked separately) | Millex-HV 0.45 µm PVDF, 25 mm OD Luer-Lok (P/N SLHVR25NB) | Disc Ø25 × 7 + inlet Ø6 × 8 + outlet Ø3 × 8 | (0.22, -0.10) lying along **X** (perpendicular to the syringe) | The 0.45 µm membrane. Placed on the bench **beside** the syringe with a 150 mm gap and a 90° orientation change so it is visually obvious that the filter is NOT pre-attached — the arm picks it up separately after Step 9 of the workflow. |
| 10 | **Receiving beaker** | [Thingiverse — greengiant1298 100ml Beaker, thing:2267](https://www.thingiverse.com/thing:2267), `.stl` (mm), 3D-print convention | Ø50 × 51 STL mesh | (0.05, -0.15) | The single receiving vessel for the filtrate. Placed in front of the arm marker (~309 mm) so a UR3 / FR3 can comfortably reach it. STL is in mm, SDF scales by 0.001 to metres. |
