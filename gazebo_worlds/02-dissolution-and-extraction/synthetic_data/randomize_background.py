#!/usr/bin/env python3
"""Swap the bench background + save the camera frames as PNGs + YOLO labels.

This is the **sixth** of the six domain-randomisation axes from
``docs/synthetic-data/features/01-detection-images-and-masks.md``.
``move_camera.py``, ``randomize_objects.py``, ``randomize_lighting.py``
and ``randomize_distractors.py`` cover axes #1, #2, #3 and #5. This
script implements axis #6 (background): the camera, every labelled
object and the lighting all stay fixed; what changes per frame is
the **colour of the surface the objects sit on**.

How we do it: at the start of each frame we spawn a thin coloured
plane just above the bench top (``z = BENCH_TOP_Z + 1 mm``), capture
a frame, then remove the plane before the next frame. From the
overhead camera the plane fully replaces the bench's wood colour
while the cylindrical objects poke up through it. From the side the
plane would look like a tablecloth — but the camera never sees that.

The five backgrounds in ``BACKGROUNDS`` were chosen to span the
common physical mats a real cell uses:

  1. white_paper   — like a fresh sheet of A4 on the bench
  2. black_mat     — anti-glare absorbing mat
  3. blue_labmat   — blue lab / anti-static mat
  4. green_cutmat  — green chroma / cutting mat
  5. grey_rubber   — neutral grey rubber mat

You can re-order the list or add more — the first ``--frames``
entries are used (with optional seeded shuffle).
...
Run order:
  ...
  Terminal 2:  python3 gazebo_worlds/02-dissolution-and-extraction/synthetic_data/randomize_background.py

Common flags:

  --frames N     number of frames (default 5, max len(BACKGROUNDS))
  --dwell S      seconds to wait after spawning (default 2)
  --seed K       if set, shuffle BACKGROUNDS deterministically before picking
...
# ---------------------------------------------------------------------------
# gz-transport / gz-msgs bindings (same fallback ladder as siblings).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Background catalogue: (slug, (r, g, b))
#
# Colours roughly match common lab / studio mats. Tweak the RGB tuples
# to taste — anything in [0, 1] per channel works. To add a sixth or
# seventh background just append to the list.
# ---------------------------------------------------------------------------
BACKGROUNDS: List[Tuple[str, Tuple[float, float, float]]] = [
    ("white_paper",  (0.93, 0.93, 0.93)),
    ("black_mat",    (0.05, 0.05, 0.05)),
    ("blue_labmat",  (0.15, 0.30, 0.65)),
    ("green_cutmat", (0.12, 0.45, 0.22)),
    ("grey_rubber",  (0.40, 0.40, 0.40)),
]
...
# ---------------------------------------------------------------------------
# Reset camera + labelled objects to SDF defaults.
# ---------------------------------------------------------------------------
...
def pick_backgrounds(args) -> List[Tuple[str, Tuple[float, float, float]]]:
    """Return the first ``args.frames`` entries of BACKGROUNDS, optionally shuffled."""
    bgs = list(BACKGROUNDS)
    if args.seed is not None:
        random.Random(args.seed).shuffle(bgs)
    if args.frames > len(bgs):
        print(f"WARNING: requested {args.frames} frames but only "
              f"{len(bgs)} backgrounds are defined — capping at "
              f"{len(bgs)}.")
    return bgs[: args.frames]
...
def main() -> None:
    ...
    p.add_argument("--seed", type=int, default=None,
                   help="if set, shuffle BACKGROUNDS deterministically first")
    ...
    log_path = save_dir / "backgrounds.jsonl"
    log_path.write_text("")

    cam_rpy = look_at(CAMERA_POSITION, LOOK_AT_TARGET)
    bg_order = pick_backgrounds(args)
    ...
    print(f"background plane     {PLANE_SIZE_X}x{PLANE_SIZE_Y} m at z={PLANE_Z}  "
          f"({PLANE_THICKNESS*1000:.1f} mm thick)")
    print(f"create service       /world/{WORLD}/create  remove: /world/{WORLD}/remove")
    print(f"backgrounds          {[s for s, _ in bg_order]}")
    ...
    # Best-effort: remove a previous-run background plane.
        ...
        with log_path.open("a") as fh:
            fh.write(json.dumps({
                ...
                "background": {
                    ...
                    "size": [PLANE_SIZE_X, PLANE_SIZE_Y, PLANE_THICKNESS],
                    "centre": [PLANE_X, PLANE_Y, PLANE_Z],
                },
            }) + "\n")
    ...
    print(f"      per-frame backgrounds -> {log_path}")
